...
import (
	...
	"github.com/metacubex/wireguard-go/tai64n"
)
...
func (device *Device) CreateMessageInitiation(peer *Peer) (*MessageInitiation, error) {
	...
	var err error
	handshake.hash = device.handshakeParams.initialHash
	handshake.chainKey = device.handshakeParams.initialChainKey
...
func (device *Device) ConsumeMessageInitiation(msg *MessageInitiation) *Peer {
	...
	mixHash(&hash, &device.handshakeParams.initialHash, device.staticIdentity.publicKey[:])
	mixHash(&hash, &hash, msg.Ephemeral[:])
	mixKey(&chainKey, &device.handshakeParams.initialChainKey, msg.Ephemeral[:])
